    ...
    internal class SetHeroClass : HeroActionHandlerBase
    {
            ...
            Action<string> onSuccess, Action<string> onFailure)
        {
            ...
            if (newClass.RequireClass && !string.IsNullOrEmpty(newClass.RequiredClassName))
            {
                ...
                {
                    onFailure($"Required class '{newClass.RequiredClassName}' not found in config!");
                    return;
                }
                int killsInRequired = BLTAdoptAHeroCampaignBehavior.Current.GetAchievementClassStat(
                    adoptedHero, requiredClass.ID, AchievementStatsData.Statistic.TotalKills);
                int needed = newClass.RequiredClassMinKills > 0 ? newClass.RequiredClassMinKills : 1;
                if (killsInRequired < needed)
                {
                    onFailure($"You need {needed} kills as {newClass.RequiredClassName} to unlock {newClass.Name}! (current: {killsInRequired})");
                    return;
                }
            }
