...
namespace BLTAdoptAHero
{
    ...
    internal class BLTAdoptAHeroCommonMissionBehavior : AutoMissionBehavior<BLTAdoptAHeroCommonMissionBehavior>
    {
        ...
        public void ApplyKillEffects(Hero hero, Agent killer, Agent killed, AgentState state, int goldPerKill, int healPerKill, int xpPerKill, float subBoost, float? relativeLevelScaling, float? levelScalingCap, float MinimumGoldPerKill)
        {
            ...
            // Prestige cumulative bonuses
            var prestigeCfg = BLTAdoptAHeroModule.CommonConfig.PrestigeConfig;
            int prestigeLevel = BLTAdoptAHeroCampaignBehavior.Current?.GetPrestigeLevel(hero) ?? 0;
            if (prestigeLevel > 0)
            {
                int goldBonusPct = prestigeCfg.GetCumulativeGoldBonusPercent(prestigeLevel);
                if (goldBonusPct > 0)
                    goldPerKill = (int)(goldPerKill * (1f + goldBonusPct / 100f));

                float xpMult = prestigeCfg.GetCumulativeXPMultiplier(prestigeLevel);
                if (xpMult > 1f)
                    xpPerKill = (int)(xpPerKill * xpMult);
            }
